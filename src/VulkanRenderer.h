...
private:
	...
	double long_frame;
	const int long_frame_count = 300;
	long frame_count = 0;					// counts the number of frames generated
	double frame_time = glfwGetTime();

	//Camera* cam;
	glm::vec3 camPosition = glm::vec3(2.0, 2.0, 2.0);
	...
	void updateFrameTimeHistory(double v);
	double getRollingFrameRate();
	...
	void updateCameraDetails(double frame_length) {
		...
		glm::vec3 camera_direction = glm::normalize(camLookAt - camPosition);
		glm::vec3 camera_axis = glm::cross(camera_direction, upwards);
		glm::vec3 tmpCamUp = glm::cross(camera_axis, camera_direction);

		/*
		TODO: Update camera rotation scaling when not using RDP
		Here we modify the camera rotation based on mouse movements.
		The X rotation should be relative to camUp.
		Meanwhile the Y rotation should be relative to the camera axis
...
#define MOUSE_MAX_TURN_VALUE 1000.
		...
		float xAngle = (float)glm::radians(camSpeedSensitivity * xpos / MOUSE_MAX_TURN_VALUE);
		float yAngle = (float)glm::radians(camSpeedSensitivity * ypos / MOUSE_MAX_TURN_VALUE);
		...
		camLookAt = camPosition + glm::normalize(camera_direction); //unnecessary, but for peace of mind
		camUp = tmpCamUp;
		...
		if(glm::length(camera_trajectory) > 0.1) {
			camera_trajectory = glm::normalize(camera_trajectory) * camSpeed * frame_length;
			camPosition = camPosition + camera_trajectory;
			camLookAt = camPosition + camera_direction;
			camUp = glm::cross(camera_axis, camera_direction);
		}
	}

	/*
	updates the GPU with new transformation matrix data

	@param currentImage - memory location for the current swap chain image, which was updated in vkAcquireNextImageKHR()
	*/
	void updateUniformBuffer(uint32_t currentImage) {
		/*
		glm::lookAt creates the camera view transformation: center is location, eye is the direction from that center point, up is up (duh)
		glm::perspective creates the frustrum transformation: V FOV in radians, aspect ratio as a float, then min cut off, the max cut off
		*/
		UniformBufferObject ubo = {};
		ubo.model = glm::mat4(1.0f);
		ubo.view = glm::lookAt(camPosition, camLookAt, camUp);
		// The final argument (perspective) is the max range cutoff for rendering. By default it is 10 "units" from the camera.
		ubo.proj = glm::perspective(glm::radians(vert_fov), swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10.0f);
		ubo.proj[1][1] *= -1;  // required for GLM, whereas +y is down and not up in Vulkan

		// here is where the buffer is actually copied over to the GPU
		// because we're using GLM the matricies copy directly
		void* data;
	...
	void initWindow() {
		...
		// GLFW has no vulkan profile, therefore we must set GLFW_CLIENT_API to GLFW_NO_API
		glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);

		// last argument is the parent window, since this is a single window program we only pass nullptr
	...
	VkSampleCountFlagBits getMaxUsableSampleCount() {
		...
		VkSampleCountFlags counts = physicalDeviceProperties.limits.framebufferColorSampleCounts < physicalDeviceProperties.limits.framebufferDepthSampleCounts ? physicalDeviceProperties.limits.framebufferColorSampleCounts : physicalDeviceProperties.limits.framebufferDepthSampleCounts;
	...
	void drawFrame() {
		...
		//pulls the next image from the swap chain
		uint32_t imageIndex;
