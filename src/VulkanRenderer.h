...
namespace tools {
	...
	*/
	...
	namespace ConvexHull {
		namespace QuickHull {
			// TODO: https://raw.githubusercontent.com/leomccormack/convhull_3d/master/convhull_3d.h
			...
			const double NOISE_THRESHHOLD = 0.0000001;
			const long MAX_FACES = 50000;


			// function declarations
			/* builds the convexhull, returning the face indices corresponding to "in_vertices" */
			void convhull_3d_build(ch_vertex* const in_vertices,            /* vector of input vertices; nVert x 1 */
			...
			void convhull_3d_export_obj(ch_vertex* const vertices,          /* vector of input vertices; nVert x 1 */
										const int nVert,                    /* number of vertices */
			...
			void convhull_3d_export_m(ch_vertex* const vertices,            /* vector of input vertices; nVert x 1 */
									  const int nVert,                      /* number of vertices */
									  int* const faces,                     /* face indices; flat: nFaces x 3 */
									  const int nFaces,                     /* number of faces in hull */
			...
			void extractVerticesFromObjFile(char* const obj_filename,       /* obj filename, WITHOUT extension */
											/* output arguments */
			...
			static double det_4x4(double* m) {
				return
 ...
 */
			...
			void convhull_3d_build(
				...
				for (j = 0; j < d; j++) {
					...
					for (i = 0; i < nVert; i++) {
						max_p = std::max(max_p, in_vertices[i].v[j]);//MAX(max_p, in_vertices[i].v[j]);
						min_p = std::min(min_p, in_vertices[i].v[j]);//MIN(min_p, in_vertices[i].v[j]);
					}
					span[j] = max_p - min_p;
				}
				...
				for (i = 0; i < nVert; i++) {
					for (j = 0; j < d; j++)
						points[i * (d + 1) + j] = in_vertices[i].v[j] + NOISE_THRESHHOLD * rand() / (float)RAND_MAX; /* noise mitigates duplicates */
		...
		namespace DivideAndConquer {
			// via http://tmc.web.engr.illinois.edu/ch3d/ch3d.pdf
			...
			Point* sort(Point P[], int n) { // mergesort
				...
				c = &head;
				do
					if (a->x < b->x) { c = c->next = a; a = a->next; } else { c = c->next = b; b = b->next; }
				while (c != NIL);
				return head.next;
			}
